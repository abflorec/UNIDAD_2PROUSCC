...
void mostrarMenu() {
    ...
    cout << "5. Registrar Consumo de Servicio\n";
    cout << "6. Registrar Pago\n";
    cout << "7. Ver Todos los Datos\n";
    cout << "8. Guardar Datos en Archivos\n";
    cout << "9. Cargar Datos de Archivos\n";
...
void agregarCliente() {
    ...
    cout << "[OK] Cliente agregado y guardado exitosamente!\n";
}

void agregarEmpleado() {
    int id;
    string nombre, apellido, dni, telefono, fechaContratacion;
    double salario;
    int cargoOpcion;
    ...
    cout << "Cargos:\n";
    cout << "0. Recepcionista\n1. Gerente\n2. Limpieza\n3. Seguridad\n";
    cout << "4. Mantenimiento\n5. Chef\n6. Camarero\n";
    cout << "Seleccione Cargo: ";
    cin >> cargoOpcion;
    
    CargoEmpleado cargo;
    if (cargoOpcion >= 0 && cargoOpcion <= 6) {
         cargo = static_cast<CargoEmpleado>(cargoOpcion);
    } else {
         cargo = CargoEmpleado::RECEPCIONISTA; // Default
    }
    ...
    Empleado nuevoEmpleado(id, nombre, apellido, dni, cargo, telefono, salario, fechaContratacion);
    empleados.push_back(nuevoEmpleado);
    
    cout << "[OK] Empleado agregado exitosamente!\n";
}
...
void agregarHabitacion() {
    ...
    cout << "[OK] Habitacion agregada exitosamente!\n";
}

void crearReserva() {
    if (clientes.empty() || habitaciones.empty()) {
        cout << "[ERROR] Error: Necesita clientes y habitaciones primero.\n";
        return;
    }
    ...
    if (habitacionIdx < 0 || habitacionIdx >= (int)habitaciones.size() || 
        ...
        cout << "[ERROR] Seleccion invalida o habitacion no disponible.\n";
        return;
    }
    ...
    cin >> fechaOut;
    cin.ignore(); // Limpiar buffer después de leer string sin espacios
    ...
    cout << "[OK] Reserva creada exitosamente! ID: #" << nuevaReserva->getIdReserva() << "\n";
}

void registrarConsumo() {
    if (reservas.empty()) {
        cout << "[ERROR] Error: No hay reservas activas.\n";
        return;
    }
    
    if (servicios.empty()) {
        cout << "[ERROR] Error: No hay servicios disponibles.\n";
        return;
    }
    
    cout << "\n--- REGISTRAR CONSUMO DE SERVICIO ---\n";
    cout << "Seleccione Reserva:\n";
    ...
    for (size_t i = 0; i < servicios.size(); i++) {
        cout << i+1 << ". " << servicios[i].getNombre() 
             << " - $" << servicios[i].getPrecio() << "\n";
    }
    ...
    if (servicioIdx < 0 || servicioIdx >= (int)servicios.size()) {
        cout << "[ERROR] Seleccion invalida.\n";
        return;
    }
    
    reservas[reservaIdx]->agregarServicio(servicios[servicioIdx]);
}

void registrarPago() {
    if (reservas.empty()) {
        cout << "[ERROR] Error: No hay reservas para pagar.\n";
        return;
    }
